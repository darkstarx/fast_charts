...
class StackedBarChartViewport extends LeafRenderObjectWidget
{
  ...
  {
    return StackedBarChanrtViewportRenderObject(
      data: data,
      animation: animation,
  ...
  void updateRenderObject(final BuildContext context,
    final StackedBarChanrtViewportRenderObject renderObject,
  )
  {
    renderObject
      ..data = data
...
class StackedBarChanrtViewportRenderObject extends RenderBox
{
  ...
  StackedBarChanrtViewportRenderObject({
  ...
  void performLayout()
  {
    ...
    final stacksClipRect = _layoutData!.stacksClipRect;
    final viewportDimension = switch (data.valueAxis) {
      Axis.horizontal => stacksClipRect.height,
      Axis.vertical => stacksClipRect.width,
    };
    final contentDimension = barThickness == null
      ? viewportDimension
      : barPadding * 2
        + barThickness! * data.stacks.length
        + barSpacing * max((data.stacks.length - 1), 0)
      ;
  ...
  void paint(final PaintingContext context, final Offset offset)
  {
    ...
    canvas.clipRect(layoutData.stacksClipRect);
    for (final label in layoutData.crossLabels) {
      final (offset, rect, paragraph) = label;
      if (barThickness != null) {
        final labelRect = rect.shift(-viewOffset);
        if (!layoutData.stacksClipRect.overlaps(labelRect)) {
          continue;
        }
      }
      canvas.drawParagraph(paragraph, offset - viewOffset);
    }
    final stacks = _stacks = _getStacks(layoutData);
    final drawnStacks = <LayoutStack>[];
    for (final entry in stacks.entries) {
      ...
      if (barThickness != null) {
        if (!layoutData.stacksClipRect.overlaps(stackRRect.outerRect)) {
          continue;
        }
      }
    ...
    if (animation == null) {
      for (final stack in drawnStacks) {
        for (final label in stack.labels) {
          final (offset, paragraph) = label;
          canvas.drawParagraph(paragraph, offset - viewOffset);
        }
      }
  ...
  LayoutData _buildLayout()
  {
    ...
    switch (data.valueAxis) {
      case Axis.horizontal:
        ...
        stacksClipRect = Offset(innerRect.left, innerRect.top + mainAxisField)
      ...
      case Axis.vertical:
        ...
        stacksClipRect = Offset(innerRect.left + mainAxisField, innerRect.top)
          ...
          if (label != null) {
            ...
            switch (label.position) {
              case LabelPosition.inside:
                stack.labels.add((lblOffset.topLeft, paragraph));
...
class LayoutData
{
  ...
  final List<(Offset, Rect, ui.Paragraph)> crossLabels;
  final Map<int, LayoutStack> stacks;

  const LayoutData({
    required this.clipRect,
    required this.stacksClipRect,
  ...
  LayoutData copyWith({
    ...
    stacksClipRect: stacksClipRect ?? this.stacksClipRect,
